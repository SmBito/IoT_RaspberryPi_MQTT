# This Python is used to create an HTTP Server that will translate HTTP requests into MQTT messages.
...
@app.route('/light/on')
def light_on():
    client.publish("s/light", "on", qos=0, retain=False)
    return "Turned ON"
@app.route('/light/off')
def light_off():
    client.publish("s/light", "off", qos=0, retain=False)
    return "Turned OFF"
